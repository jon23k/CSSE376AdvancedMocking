...
namespace CommandClientVisualStudioTest
{
    ...
    public class AdvancedMockTests
    {
        ...
        public void TestUserExitCommandWithoutMocks()
        {
            ...
            using (mocks.Ordered())
            {
                myStream.Write(commandBytes, 0, 4);
                myStream.Flush();
                myStream.Write(ipLength, 0, 4);
                myStream.Flush();
                myStream.Write(ip, 0, 9);
                myStream.Flush();
                myStream.Write(metaDataLength, 0, 4);
                myStream.Flush();
                myStream.Write(metaData, 0, 2);
                myStream.Flush();
            }
            mocks.ReplayAll();
            CMDClient client = new CMDClient(null, "Bogus network name");
